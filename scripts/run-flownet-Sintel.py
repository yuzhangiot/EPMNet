...
parser.add_argument('root', help='the root dir of data,contain images, gt and results')
parser.add_argument('dataset', help='data set: clean, final...')
...
# if(not os.path.exists(args.root)): raise BaseException('root does not exist: '+args.root)
...
def getImages(path, dataset):
    # list = []
    imgPath = path + dataset
    list = [f for f in listdir(imgPath)]
    return list


# ops = readTupleList(args.listfile)

ops = getImages(args.root, args.dataset)
...
for ent in ops:
    ...
    img_root = args.root + args.dataset + "/" + ent
    dirname = args.root + "results/" + args.dataset + "/" + ent
    os.system("mkdir -p " + dirname)
    fileNum = len([name for name in os.listdir(img_root)])
    print(img_root)
    print(fileNum)
    for idx in range(1,fileNum):
        img0_path = img_root + "/frame_" + str(idx).zfill(4) + ".png"
        img1_path = img_root + "/frame_" + str(idx + 1).zfill(4) + ".png"

        out_path = args.root + "results/" + args.dataset + "/" + ent + "/frame_" + str(idx).zfill(4) + ".flo"

        num_blobs = 2
        input_data = []
        img0 = misc.imread(img0_path)
