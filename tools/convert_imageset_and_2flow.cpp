...
public:
    ...
    std::string flowL_filename;
    std::string flowR_filename;
    ...
    ImagePair(std::string line)
    {
        clrptr();
        stringstream s(line);
        s >> img1_filename >> img2_filename >> flowL_filename >> flowR_filename;
    }
    ~ImagePair()
    {
        if(data) delete data;
    ...
    bool has_flow() const { return flowL_filename!=""; }
    ...
    void read_data()
    {
        ...
        float* flowL_data=0;
        float* flowR_data=0;
        if(hasEnding(flowL_filename, ".pfm"))
            readPfmFile(flowL_filename,flowL_data,xSize,ySize);
        else
            readFloFile(flowL_filename,flowL_data,xSize,ySize);
        if(hasEnding(flowR_filename, ".pfm"))
            readPfmFile(flowR_filename,flowR_data,xSize,ySize);
        else
            readFloFile(flowR_filename,flowR_data,xSize,ySize);
        ...
        data_size=  3*width*height              // Image 1
                   ...
                   +2*2*width*height            // FlowL
                   +2*2*width*height ;          // FlowR
        ...
        for (int j = 0; j<2*width*height; j++) {
            ...
            if(has_flow())
            {
                if(isnan(flowL_data[j]))
                    value=std::numeric_limits<short>::max();
                else
                    value=flowL_data[j]*32; // = flo_data[j]*32.768/1024;
            }
        ...
        for (int j = 0; j<2*width*height; j++) {
            ...
            if(has_flow())
            {
                if(isnan(flowR_data[j]))
                    value=std::numeric_limits<short>::max();
                else
                    value=flowR_data[j]*32; // = flo_data[j]*32.768/1024;
            }
...
int main(int argc, char** argv) {
  ...
  //for (int line_id = 0; line_id < 1; ++line_id) {
    ...
    snprintf(key_cstr, kMaxKeyLength, "%08d_%s", line_id,
        lines[line_id].flowL_filename.c_str());
    string value;
