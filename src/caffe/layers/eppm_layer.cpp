...
void EppmLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {
  // Configure the kernel size, padding, stride, and inputs.
  EppmParameter eppm_param = this->layer_param_.eppm_param();
  
  CHECK(eppm_param.has_kernel_size()) << "Filter kernel_size is not set";
  CHECK(eppm_param.has_max_displacement()) << "Max displacement is required.";
  
  kernel_size_ = eppm_param.kernel_size();
  if(kernel_size_ % 2 == 0) LOG(FATAL) << "Odd kernel size required";
  
  max_displacement_ = eppm_param.max_displacement();
  pad_size_ = eppm_param.pad();
  stride1_ = eppm_param.stride_1();
  stride2_ = eppm_param.stride_2();
  
  do_abs_ = eppm_param.do_abs();
  
  eppm_type_ = eppm_param.eppm_type();
  ...
  LOG(INFO) << "EPPM type: " << eppm_type_;
  
}

template <typename Dtype>
void EppmLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {
...
template <typename Dtype>
void EppmLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {
  NOT_IMPLEMENTED;
}

template <typename Dtype>
void EppmLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
...
INSTANTIATE_CLASS(EppmLayer);
REGISTER_LAYER_CLASS(Eppm);
